...
userSchema.method("isRole", function isRole(roleName: string, only = false) {
  return only
    ? this.roles.includes(roleName) && this.roles.length === 1
    : this.roles.includes(roleName);
});
userSchema.method(
  "hasRoles",
  function hasRoles(roleNames: string[], only = false) {
    if (only)
      return (
        roleNames.every((roleName) => this.roles.includes(roleName)) &&
        roleNames.length === this.roles.length
      );
