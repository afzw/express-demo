...
export async function signIn(req: Request, res: Response) {
  const signInProfile: Public.SignInProfile = _.pick(req.body, 'email', 'password')
  ...
  req.login(user, async (error: any) => {
    ...
    if (saveSessionErr) return res.status(500).send(`登录失败！记录session信息失败！详情：${saveSessionErr}`)
...
export async function signUp(req: Request, res: Response) {
  const signUpProfile = _.pick(req.body, signUpFields)
